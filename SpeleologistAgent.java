...
public class SpeleologistAgent extends Agent {
    ...
    private class WumpusWorldFinder extends Behaviour {
        ...
        public void action() {
            ...
            sd.setType(EnvironmentAgent.Constants.WUMPUS_WORLD_TYPE);
        ...
        public void action() {
            switch (step) {
                case 0:
                    ...
                    cfp.setContent(EnvironmentAgent.Constants.GO_INSIDE);
                    cfp.setConversationId(EnvironmentAgent.Constants.WUMPUS_WORLD_DIGGER_CONVERSATION_ID);
                    cfp.setReplyWith("cfp" + System.currentTimeMillis());
                    myAgent.send(cfp);
                    mt = MessageTemplate.and(MessageTemplate.MatchConversationId(EnvironmentAgent.Constants.WUMPUS_WORLD_DIGGER_CONVERSATION_ID),
    ...
    private class NavigatorAgentFinder extends Behaviour {
        ...
        public void action() {
            DFAgentDescription template = new DFAgentDescription();
            ServiceDescription sd = new ServiceDescription();
            sd.setType(EnvironmentAgent.Constants.NAVIGATOR_AGENT_TYPE);
            template.addServices(sd);
            try {
                DFAgentDescription[] result = DFService.search(myAgent, template);
                if (result != null && result.length > 0) {
        ...
        public void action() {
            switch (step) {
                case 0:
                    ...
                    inform.setContent(EnvironmentAgent.Constants.ADVICE_PROPOSAL);
                    inform.setConversationId(EnvironmentAgent.Constants.NAVIGATOR_DIGGER_CONVERSATION_ID);
                    inform.setReplyWith("inform" + System.currentTimeMillis());
                    System.out.println("Speleologist: " + EnvironmentAgent.Constants.ADVICE_PROPOSAL);
                    myAgent.send(inform);
                    mt = MessageTemplate.and(MessageTemplate.MatchConversationId(EnvironmentAgent.Constants.NAVIGATOR_DIGGER_CONVERSATION_ID),
                    ...
                    if (reply != null) {
                        if (reply.getPerformative() == ACLMessage.REQUEST) {
                            if (parseNavigatorMessageRequest(reply.getContent())) {
                                ...
                                req.setContent(EnvironmentAgent.Constants.GAME_INFORMATION);
                                req.setConversationId(EnvironmentAgent.Constants.WUMPUS_WORLD_DIGGER_CONVERSATION_ID);
                ...
                case 2:
                    ...
                    if (reply != null) {
                        if (reply.getPerformative() == ACLMessage.INFORM) {
                            ...
                            mes.setContent(EnvironmentAgent.Constants.INFORMATION_PROPOSAL_SPELEOLOGIST + rep);
                            mes.setConversationId(EnvironmentAgent.Constants.NAVIGATOR_DIGGER_CONVERSATION_ID);
                            mes.setReplyWith("mes" + System.currentTimeMillis());
                            System.out.println("Speleologist: " + EnvironmentAgent.Constants.INFORMATION_PROPOSAL_SPELEOLOGIST + rep);
                            myAgent.send(mes);
                            mt = MessageTemplate.and(MessageTemplate.MatchConversationId(EnvironmentAgent.Constants.NAVIGATOR_DIGGER_CONVERSATION_ID),
                        ...
                        if (reply.getPerformative() == ACLMessage.PROPOSE) {
                            ...
                            switch (action) {
                                case Commands.TURN_LEFT:
                                    content = EnvironmentAgent.Constants.SPELEOLOGIST_TURN_LEFT;
                                    break;
                                case Commands.TURN_RIGHT:
                                    content = EnvironmentAgent.Constants.SPELEOLOGIST_TURN_RIGHT;
                                    break;
                                case Commands.MOVE_FORWARD:
                                    content = EnvironmentAgent.Constants.SPELEOLOGIST_MOVE_FORWARD;
                                    break;
                                case Commands.GRAB:
                                    content = EnvironmentAgent.Constants.SPELEOLOGIST_GRAB;
                                    break;
                                case Commands.SHOOT:
                                    content = EnvironmentAgent.Constants.SPELEOLOGIST_SHOOT;
                                    break;
                                case Commands.CLIMB:
                                    content = EnvironmentAgent.Constants.SPELEOLOGIST_CLIMB;
                                    break;
                                default:
                            ...
                            prop.setConversationId(EnvironmentAgent.Constants.WUMPUS_WORLD_DIGGER_CONVERSATION_ID);
                ...
                case 4:
                    ...
                    if (reply != null) {
                        ...
                        switch (answer) {
                            case EnvironmentAgent.Constants.FAIL_MESSAGE:
                                System.out.println("Speleologist: You failed!");
                                step++;
                                doDelete();
                                break;
                            case EnvironmentAgent.Constants.OK_MESSAGE:
                                System.out.println("Speleologist: Wumpus world answers OK.");
                                step = 0;
                                break;
                            case EnvironmentAgent.Constants.WIN_MESSAGE:
